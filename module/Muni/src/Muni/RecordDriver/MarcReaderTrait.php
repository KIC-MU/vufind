...
trait MarcReaderTrait
{
    ...
    protected function getPublicationInfo($subfield = 'a')
    {
        ...
        // copyright, publication, and production places (and ignore copyright places
        // if publication places are present and ignore publication places if production
        // places are present). This behavior is designed to be consistent with default
        // SolrMarc handling of names/dates.
        $prodResults = $pubResults = $copyResults = [];
        ...
        if (is_array($fields)) {
            foreach ($fields as $currentField) {
                ...
                if (!empty($currentVal)) {
                    ...
                    case '0':
                        $prodResults = array_merge($prodResults, $currentVal);
                        break;
                    case '1':
        ...
        if (count($prodResults) > 0) {
            return $replace260 ? $prodResults : array_merge($results, $prodResults);
        } elseif (count($pubResults) > 0) {
