 ...
 * @link     https://vufind.org Main Page
 */
class AuthorityRecommend implements \VuFind\Recommend\RecommendInterface
{
    /**
     * User search query
     *
    ...
    protected function addUseForHeadings()
    {
        ...
        // above
        $this->recommendations['usefor'] = [];
        foreach ($this->performSearch($params) as $result) {
            $this->recommendations['usefor'][] = $result->getBreadcrumb();
        }
    }
        ...
        foreach ($this->performSearch($params) as $result) {
            foreach ($result->getSeeAlso() as $seeAlso) {
                ...
                if (!$this->fuzzyCompare($seeAlso, $this->lookfor)) {
                    $this->recommendations['seealso'][] = $seeAlso;
                }
            }
        }
    }
    ...
    {
        $returnValue = [];
        foreach ($this->recommendations as $key => $value) {
            $returnValue[$key] = array_unique($value);
        }
        return $returnValue;
    }
